...
describe('Products Repository', () => {
  ...
  describe('getAllProducts', () => {
    it('retourne tous les produits avec image_paths en tableau', async () => {
      mockDbAll.mockResolvedValue([
        { id: 1, label: 'Produit A', price: 10, category: 'cat1', image_paths: '/uploads/a.jpg,/uploads/b.jpg' },
        { id: 2, label: 'Produit B', price: 20, category: 'cat2', image_paths: null },
      ]);
      ...
      expect(products[0].image_paths).toEqual(['/uploads/a.jpg', '/uploads/b.jpg']);
  ...
  describe('getProductById', () => {
    it('retourne le produit avec ses images', async () => {
      mockDbGet.mockResolvedValue({ id: 1, label: 'Produit A', price: 10, category: 'cat1' });
      mockDbAll.mockResolvedValue([
        { id: 1, filename: 'a.jpg', path: '/uploads/a.jpg', mime_type: 'image/jpeg' },
      ]);

      const product = await getProductById(1);

      expect(mockDbGet).toHaveBeenCalledWith('SELECT * FROM products WHERE id = ?', [1]);
      expect(product).toEqual({
        id: 1,
        label: 'Produit A',
        price: 10,
        category: 'cat1',
        images: [{ id: 1, filename: 'a.jpg', path: '/uploads/a.jpg', mime_type: 'image/jpeg' }],
      });
    });
  ...
  describe('deleteProduct', () => {
    it('supprime un produit par id', async () => {
      ...
      expect(mockDbRun).toHaveBeenCalledWith('DELETE FROM products WHERE id = ?', [1]);
      expect(result.changes).toBe(1);
    });
  });
  ...
  describe('getProductImages', () => {
    it('retourne les images d\'un produit', async () => {
      const images = [
        { id: 1, filename: 'a.jpg', path: '/uploads/a.jpg', mime_type: 'image/jpeg' },
        { id: 2, filename: 'b.png', path: '/uploads/b.png', mime_type: 'image/png' },
      ];
  ...
  // Suppression d'une image sur un produit
  ...
  describe('deleteProductImages', () => {
    it('supprime toutes les images d\'un produit', async () => {
      ...
      expect(mockDbRun).toHaveBeenCalledWith('DELETE FROM product_images WHERE product_id = ?', [1]);
