...
const parseBase64Image = (dataUrl) => {
  ...
  if (!ALLOWED_MIME_TYPES.has(mimeType)) {
    const err = new Error(
      `Type d'image non supporté : ${mimeType}. Types acceptés : jpeg, png, webp, gif`,
    );
    err.status = 422;
    throw err;
  }
...
export const listProducts = () => productsRepo.getAllProducts();
...
export const updateProduct = async (
  id,
  { label, description, price, category, images, removedImageIds },
) => {
  ...
  if (removedImageIds.length > 0) {
    for (const imageId of removedImageIds) {
      const img = await productsRepo.getProductImageById(Number(imageId));
      if (img && img.product_id === id) {
        await productsRepo.deleteProductImageById(img.id);
        await removeImageFiles([img]);
      }
    }
  }
